...
%   [H, THETA, RHO] = HOUGH(F, DTHETA, DRHO) computes the hough transform
%   of the image F. DTHETA specifies the spacing (in degrees) of the Hough
...
%   matrix. It is NRHO-by-NTHETA, where NRHO=2*ceil(norm(size(F))/DRHO)-1,
%   and NTHETA = 2*ceil(90/DTHETA). Note that if 90/DTHETA is not an
%   integer, the actual angle spacing will be 90/ceil(90)/DTHETA.
%
...
%   Take advantage of the fact that the SPARSE function, 
%   which constructs a sparse matrix, accumulates values when input indices 
%   are repeated. That's the behavior we want for the Hough transform. We want
%   the output to be a full (nonsparse) matrix, however, so we call
