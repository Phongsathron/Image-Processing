...
%   HOUGHPEAKS detects peaks in Hough transform.
%   [R, C, HNEW] = HOUGHPEAKS(H, NUMPEAKS, THRESHOLD, NHOOD) detects peaks
%   in the Hough transform matrix H. NUMPEAKS specifies the maximum number 
%   of peak locations to look for. Values of H below THRESHOLD will not be
...
%   elements of NHOOD must be positive, odd integers. R and C are the row
%   and column coordinates of the identified peaks. HNEW is the Hough
%   transform with peak neighborhood suppression.
%
...
if nargin < 4
    nhood = size(h)/50;
    % Make sure the neightborhood size is odd.
...
while ~done
    ...
    if hnew(p, q) >= threshold
        ...
        % Convert to linear indices to zeros out all the values.
